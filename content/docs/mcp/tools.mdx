...
| `discover_repositories` | Read | List the repositories that publish generators in the open, so the agent can name one for the user to connect. What is listed is published by its authors and reviewed by nobody. |
