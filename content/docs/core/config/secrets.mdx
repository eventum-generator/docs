...
| `eventum mcp` | `--keyring-cryptfile` flag. Enables the agent's `list_secrets` to report stored secret **names**. |

<Callout type="info">
An [MCP](/docs/mcp) agent can call `list_secrets` to discover which secret **names** exist — names only, never values — so it references them correctly in a config. It cannot add, change, or read secret values; manage those with `eventum-keyring` (below). Point the stdio server at the cryptfile with [`eventum mcp --keyring-cryptfile`](/docs/core/cli/eventum-mcp).
</Callout>
