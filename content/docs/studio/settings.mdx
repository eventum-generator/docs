...
Renaming a secret states what refers to it first, since the two kinds of referrer are affected differently. A [connected repository](/docs/studio/repositories) authenticating with the secret is repointed at the new name, so it keeps working; a project reading it as `${secrets.name}` keeps the old name in its configuration and has to be edited.
