...
The output selector starts on the plugin open in the Inspector. Pick any configured output and press **Load** to copy its formatter into the preview. A plugin without an explicit `formatter` loads the default it uses at runtime. The status reads **Loaded** while the preview matches the selected output and **Custom** after you change it.

Loading does not change `generator.yml`, and the copied settings remain editable. To send a custom formatter back to the selected output, press **Apply to output** and confirm the replacement. This updates the Studio draft while preserving the plugin's other settings. Use the main **Save** action to persist it to the project.
