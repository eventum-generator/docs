...
`--max-concurrency` limits [concurrent write operations across all output plugins](/docs/core/concepts/output#concurrency) — here, that's the `http` plugin's batch writes. The `json-batch` formatter configured above turns each batch into a single HTTP request carrying the whole batch as one JSON array (see [batch vs per-request delivery](/docs/tutorials/delivery/http#batch-vs-per-request-delivery)), so one write is one request: `--max-concurrency 10` caps the run at 10 concurrent requests in flight, not 10 batches' worth of per-event requests. The 5,000 events split into 50 batches of 100, with up to 10 of those batches' requests in flight at once. Swapping `formatter` to `json` instead would fan each batch out into one request per event, multiplying `--batch.size` by `--max-concurrency` into the concurrent-request ceiling (1,000, at these settings) — but this lesson's `json-batch` config keeps it to one request per batch throughout. Adjust `--batch.size` and `--max-concurrency` together to find your API's breaking point without overshooting it by accident.
