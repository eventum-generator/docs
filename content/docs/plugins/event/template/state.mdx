...
| `release_if_held()` | Release every hold this template has on the state lock, and return how many were released. |
...
The lock is never held between events: a hold left behind — including one left by a render that failed before reaching `release()` — is released once the event is over, so it cannot block other generators.
