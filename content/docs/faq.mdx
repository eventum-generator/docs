...
- **`batch.delay`** — maximum time span of the event timestamps one batch covers. Timestamps that have already passed are grouped by `batch.size` instead.
...
<Accordions>
<Accordion title="How do I apply a configuration change without restarting the process?">
