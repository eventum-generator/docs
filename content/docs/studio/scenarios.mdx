...
The diagram maps the global state the instances exchange: which generator writes a key, and which ones read it. It is present once any generator in the scenario calls `globals.set()` or `globals.get()` in a template. Keys a `script` plugin reads or writes are not mapped, though their values appear in the global state panel.
