...
`framing` on the `tcp` output picks between the two. It defaults to `delimiter` — non-transparent framing with whatever `separator` holds — and that is what to keep unless a collector's own documentation specifically calls for octet-counting. RFC 6587 itself flags why octet-counting exists at all: a trailing delimiter only works if the message never contains that same character, and nothing stops a syslog message from carrying an embedded `LF`. Where a collector asks for the length-prefixed form, `framing: octet_counting` writes it:
...
The prefix counts the bytes of the encoded message, as RFC 6587 requires, so a message carrying multi-byte UTF-8 is framed by its real length rather than by its character count. `separator` has no place beside it — the length is what marks the end of each message now — and a config that sets both is rejected before the run starts.
