...
In the tool tables, the **Access** column marks each tool **Read** or **Write**. Write tools change files or control generators, and are gated: they require `server.mcp.allow_write` over HTTP, and over stdio are available unless you pass `--read-only`.
...
| `run_generator` | Write | Run a saved generator once to its configured outputs, stopping when it finishes or hits a time or event limit. |

<Callout type="info">
`validate_generator` and the `preview_*` tools run against the real Eventum pipeline, so the agent sees exactly what a real run would produce. See [how it works](/docs/mcp/how-it-works#the-loop).
</Callout>
...
| `register_generator` | Write | Register a generator the agent authored with the running server, so it can be started and is restored after a restart. |
