---
title: "SIEM Test Data: Windows Security Events → OpenSearch"
description: Generate SIEM test data — realistic Windows Security Event Log sessions indexed into OpenSearch — for detection development, parser validation, and analyst training without touching production.
---

SIEM test data is synthetic security telemetry — logons, privilege use, process activity, and similar events — shaped to resemble what a real environment produces, without exposing production systems or capturing genuine user activity. Teams generate it to develop and tune detection rules against realistic event sequences, to validate that a SIEM's parsers and ingestion pipeline handle a given log source correctly, and to train analysts on realistic alert volume and structure — none of it requires running a live attack or touching a production system.
...
<Accordions>
<Accordion title="Do I need a live SIEM or real attacks to use this?">
...
<Accordion title="Which log sources can I generate for SIEM besides Windows Security?">
This walkthrough builds a Windows Security generator, but the same input-template-output pattern applies to other sources. The [formats field guide](/docs/tutorials/formats) covers the Windows Event Log, CEF, LEEF, and syslog shapes structurally, and the [Eventum Hub](/hub) ships ready-made generators for network, endpoint, identity, and cloud security telemetry alongside [windows-security](/hub/windows-security) and [windows-sysmon](/hub/windows-sysmon).
</Accordion>
<Accordion title="Can I index into Elasticsearch or another backend instead of OpenSearch?">
...
<Accordion title="How do I add more event types?">
...
- The [SIEM scenarios pillar](/docs/tutorials/scenarios) for the broader detection-testing picture and other security telemetry sources
