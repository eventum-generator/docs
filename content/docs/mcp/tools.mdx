...
<Callout type="warn">
An archive travels inside the tool call, so it lands in the agent's context. `export_generator` and `import_generator` therefore refuse an archive over 128 KiB and point at the [REST API](/docs/api), which carries a project of any size.
</Callout>
