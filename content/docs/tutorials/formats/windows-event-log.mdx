...
With `start` anchored to midnight and a one-day period, `mode: 0.5` lands at noon; `left: 0.15` and `right: 0.9` confine every event to between 03:36 and 21:36, with density tapering linearly toward each edge of that window. `ratio: 2000` sets about 2,000 logon-family events per day before the randomizer's ±25% variance is applied.
