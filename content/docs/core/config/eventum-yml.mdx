...
| `server.mcp.allow_write` | boolean | `false` | — | Allow write tools — file writes and starting, stopping, and registering generators. When `false`, the endpoint is read-only: discovery, validation, and preview only. |
| `server.mcp.path` | string | `"/mcp"` | Non-empty. | Mount path for the MCP endpoint. |
