---
title: "Apache log format: Common Log Format, Combined Log Format, and Nginx"
...
Testing a log shipper's parsing rules, a WAF signature tuned against request-line shape, or a billing job that reconciles bytes served all need access-log lines shaped exactly the way a real web server writes them. A live Apache or nginx instance can produce them, but its output arrives on whatever schedule its actual traffic sets, and standing one up just to capture a redirect, a 404, or a response with an empty body is slow for what it returns. Both vendors document the format precisely, but neither ships more than a single illustrative line — nowhere near the varied stream a parser actually needs to be exercised against.

Eventum renders that stream directly from a template — client address, request line, status, and byte count in the exact field order Apache's Combined Log Format defines, the same order nginx's matching default uses — and writes each line to a file exactly as produced, so a parsing rule, a WAF signature, or a billing script can be exercised against realistic access-log traffic without a real web server anywhere in the loop.
...
Apache's own reference example ties every field to a value:
...
The generator below models a small storefront's traffic — page and asset requests that mostly succeed, a share of `304 Not Modified` responses from conditional requests, and a rarer mix of client and server errors — rendering each one as a single Combined-format line. Apache Combined and nginx's `combined` produce the same line for the same request, so one template covers what either vendor would actually write; nothing in the rendered line itself says which one produced it.
...
Fields are filled with [`module.rand`](/docs/plugins/event/template/modules), the [template](/docs/plugins/event/template) event plugin's built-in randomization module. `templates/access-success.jinja` renders the common case — mostly `200`s, a share of zero-byte `304`s that exercise the caveat above directly, and an occasional `301`:
...
Running the generator in [sample mode](/docs/core/concepts/generator#sample-mode) for a short burst produced consistent Combined-format lines end to end. Seven consecutive lines from an actual run:
...
<Accordions>
<Accordion title="Does Eventum's Hub have an Apache or nginx generator?">
Yes — the [web-apache](/hub/web-apache) and [web-nginx](/hub/web-nginx) generators model both servers in depth, including bot traffic, scanner probes, and correlated error entries. Neither one emits the raw text line this page teaches, though: both model the *downstream* shape a Filebeat or Elastic Agent module produces after parsing that line — an ECS-mapped JSON document tagged `apache.access` or `nginx.access` — since that's the form most log pipelines actually index. See the [ECS lesson](/docs/tutorials/formats/ecs) for what that parsed shape looks like, and use the templates above to generate the raw line itself.
</Accordion>
...
Not from the bytes column alone. A bare `-` is Apache's convention (`%b`), a bare `0` is nginx's (`$body_bytes_sent`, compatible with Apache's own `%B`), but nothing in the line names which vendor produced it either way. In practice, whatever already tells a pipeline which host or service shipped a given file is what selects the right convention — a parser meant to handle both should treat `-` and `0` in that column as the same underlying fact, a response with no body.
</Accordion>
</Accordions>
