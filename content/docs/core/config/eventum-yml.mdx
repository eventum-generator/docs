...
`delay` limits how far a batch stretches along the timeline of the events it carries. In live mode that is the delivery lag batching adds, since a batch is delivered once its last event comes due: the first event of a batch waits at most `delay` seconds past its own timestamp. Events whose timestamps have already passed — a generator started on a past time range with `skip_past` disabled, or one running behind real time — have no such lag left to bound, so a generator draining that backlog groups them by `size` alone.
