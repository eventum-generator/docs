...
<Callout type="info">
For best performance, use the **free-threaded build** (Python 3.14t). It disables the GIL, allowing multiple generators to run truly in parallel. Install it with `uv python install 3.14t`.
</Callout>
...
- **Any platform with uv:** `uv python install 3.14` (uv manages Python versions for you)
