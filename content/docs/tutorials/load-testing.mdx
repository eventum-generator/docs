...
Load test data is the request stream a load test actually needs to be useful: not one payload sent as fast as possible, but a realistic mix of operations, identifiers, and body shapes, released at whatever rate the test calls for. A load test built on the wrong kind of data measures the wrong thing — how fast a target answers the same cached request a thousand times, not how it holds up against the shape of traffic a real deployment receives.
...
| `--max-concurrency 10` | Up to 10 batch writes in flight at once — see below |

`--max-concurrency` limits [concurrent write operations across all output plugins](/docs/core/concepts/output#concurrency), not concurrent requests: the `http` plugin fires every event in a batch as its own concurrent request, so with `--batch.size 100` and `--max-concurrency 10`, as many as 1,000 requests can be in flight at once in the worst case, spread across up to 10 batches. The generator fires 5,000 requests grouped into 50 batches of 100 this way. Adjust `--batch.size` and `--max-concurrency` together to find your API's breaking point without overshooting it by accident.
...
<Callout type="info">
No test API to point at yet? The `http` output creates its client lazily and never tests `url` at startup, so an unreachable endpoint doesn't stop the run. Every write just fails in the background, logged rather than raised — the `stdout` swap above works whether or not `url` resolves to anything real.
</Callout>
...
- The [Eventum Hub](/hub) for ready-made generators to point at your own API instead of building one from scratch
