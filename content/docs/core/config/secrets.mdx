...
<Callout type="info">
An [MCP](/docs/mcp) agent can call `list_secret_names` to discover which secret **names** exist — names only, never values — so it references them correctly in a config. It cannot add, change, or read secret values; manage those with `eventum-keyring` (below). Point the stdio server at the cryptfile with [`eventum mcp --keyring-cryptfile`](/docs/core/cli/eventum-mcp).
</Callout>

## Naming a secret

A configuration reads a secret as `${secrets.<name>}` and is rendered to substitute it, so the name is read as an expression: words of letters, digits and `_`, separated by `.`, not starting with a digit.

| Name | |
|---|---|
| `db_password`, `API_TOKEN`, `aws.access_key` | accepted |
| `db-password`, `1token`, `my key`, `a/b` | refused |

A name outside that could be stored but never read back, so it is refused wherever a secret is written — the CLI, the API and Studio. Names already in a keyring are left as they are; rename one to bring it back into use.
