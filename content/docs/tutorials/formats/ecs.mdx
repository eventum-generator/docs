---
title: "ECS fields: how Elastic Common Schema structures an event, and how to generate one"
...
The generator below models SSH access to a small fleet of Linux servers — ssh logs are one of the two sources ECS's own authentication category description names as typical, alongside Windows event logs — producing a common successful logon and a rarer failed attempt, each a complete ECS authentication event.

### The templates

Because an ECS event nests several field sets inside one document — `event`, `host`, `user`, `source`, `related` — each template below builds the event as a single Jinja mapping and serializes the whole structure at once with the built-in `tojson` filter, the same approach the [OCSF lesson](/docs/tutorials/formats/ocsf) uses for its own nested event shape. Fields are filled with [`module.rand`](/docs/plugins/event/template/modules), the [template](/docs/plugins/event/template) event plugin's built-in randomization module.
...
Both events carry the same `event.category` and `event.type` — `authentication` and `start` — since both describe the same stage of the same process; only `event.outcome`, and `event.reason` on the failed one, change to reflect what actually happened. `related.ip` carries two addresses on every event, the connecting client and the host itself, and `related.user` and `related.hosts` still write their single value as a one-element array, exactly as the rule requires.
...
- The [formats field guide](/docs/tutorials/formats) covering Windows Event Log, CEF, LEEF, syslog, and NDJSON
