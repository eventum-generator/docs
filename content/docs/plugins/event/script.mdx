...
### Correlating with other generators through global state
...
<Callout type="info">
Global state written by a script is not shown in the data flow diagram of [Scenarios](/docs/studio/scenarios), which is built from templates. The values themselves appear in the global state panel of a scenario the generator belongs to.
</Callout>
