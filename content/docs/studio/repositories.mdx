...
| **Secret** | No | The [secret](/docs/core/config/secrets) holding the password or access token of a private repository. The field names a secret of the keyring — not its value, and not a `${secrets.*}` reference. |
...
Each repository carries a badge for whether it answered the last time it was asked: **Reachable**, **Unreachable** with the reason, or **Not checked** until the page asks. The check is a request for the references the repository publishes and transfers nothing of it.
...
The line above the table names how many generators the repository publishes, the revision the catalog was read from with its author and date, and the moment it was read. The search beside it narrows the table by name, title and summary. **Refresh** reads the catalog again, which is how a generator added to the repository after the catalog was first read appears in the list.
...
An installed project carries a `.eventum-source.yml` file naming where it came from: the repository, the generator, the commit, and the content of that generator at that commit. The catalog reads it back, so a generator already in the workspace is marked **installed** by its origin rather than by a name that happens to match — a project renamed afterwards keeps the mark, and an unrelated project sharing a name does not get one.

The content is what tells the two apart: when the repository publishes the generator differently from what the project holds, the entry is marked **update available**. Installing it again under another name puts the two side by side; nothing overwrites the project you already have.
