...
<Callout type="info">
The flat `server.ui_enabled` and `server.api_enabled` keys are deprecated aliases for `server.ui.enabled` and `server.api.enabled`. They still work but print a deprecation notice at startup; setting a flat key together with its nested counterpart is an error.
</Callout>
