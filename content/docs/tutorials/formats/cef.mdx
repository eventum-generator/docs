...
| Name | A short, human-readable description. It should not repeat information already carried by another field: "Port scan", not "Port scan from 10.0.0.1 targeting 20.1.1.1". |
...
The Extension carries the event's data as `key=value` pairs, space-separated, in any order. Keys come from ArcSight's own Extension Dictionary — several hundred predefined names — of which a handful account for most events:
...
- **Equals sign (`=`)** must be escaped as `\=` inside an extension value; equals signs in the header need no escaping.
...
- The [formats field guide](/docs/tutorials/formats) covering Windows Event Log, LEEF, and syslog
