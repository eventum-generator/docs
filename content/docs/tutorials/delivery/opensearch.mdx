...
A dashboard, a detection rule, or an ingestion benchmark all need the same raw material: a realistic stream of log documents already sitting in an OpenSearch index. Waiting for production traffic to accumulate takes time most projects don't have, and copying real logs into a test cluster means carrying whatever they contain — IP addresses, session tokens, request bodies — into an environment built for testing, not for holding production data. Searching for a way around this mostly turns up ingestion tutorials that assume the logs already exist somewhere, or a Python script that fakes Apache access-log lines into a file — useful for the lines themselves, silent on how they're supposed to reach an index.

Eventum closes that gap with a native `opensearch` output plugin: point it at a cluster, describe a document's shape in a template, and it sends every generated event to the index over the bulk API as it's produced — continuously in real time, or as a single finite batch for a one-off test dataset.
...
Every index also has a **mapping** — the schema that says what fields a document may carry and how each one is stored: a full-text `text` field, an exact-match `keyword`, a `date`, a number. Without one, OpenSearch infers a mapping from the first documents it sees, a convenience that also means a field can silently end up the wrong type — most often a timestamp read as `text` instead of `date`, which quietly breaks range queries and time-based dashboards. Production setups usually create the index with an explicit mapping before sending it any data; for evaluation or a dashboard prototype, the inferred mapping is normally enough to start from.
...
`@timestamp` is the field OpenSearch Dashboards looks for by default when it suggests a time field for a new index pattern, so keeping that name avoids a manual remap later.

### The generator config

A [cron](/docs/plugins/input/cron) input ticks once a second and emits four timestamps per tick, so the single `access_log` template renders four events per batch — enough for one bulk request to carry more than one document at a time. The [opensearch](/docs/plugins/output/opensearch) output indexes them:
...
Running the generator above produces a steady stream of bulk requests, four documents each, matching the four events emitted by every cron tick. Two events from an actual run, before indexing:
...
`_source` is exactly the document the template rendered. OpenSearch stores it unchanged and layers `_index`, the assigned `_id`, and a `_version` around it for retrieval — the same three fields any client gets back from a search hit against `web-access-logs`.
...
- The [SIEM scenario track](/docs/tutorials/scenarios) for a complete stateful generator indexing Windows Security events into OpenSearch
- The [opensearch output reference](/docs/plugins/output/opensearch) for every connection, authentication, and TLS parameter
- The [formats field guide](/docs/tutorials/formats) for generating other log shapes — Windows Event Log, CEF, LEEF, syslog — headed to the same or a different destination
