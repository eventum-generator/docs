...
Eventum's [file](/docs/plugins/output/file) output, paired with its [json](/docs/plugins/formatters#json) formatter, writes exactly that shape directly from a template: every event lands on its own line as a compact, independently valid JSON object, with nothing wrapping the stream and no punctuation to rewrite as new events arrive — so a log shipper, a bulk-ingest endpoint, or a `jq` filter can be exercised against a realistic NDJSON stream without a live source anywhere in the loop.
...
- The [IoT sensor telemetry](/docs/tutorials/iot-telemetry) tutorial for this same shape written through the `stdout` output instead of `file`
