---
title: "Windows Event Log & Sysmon: generate realistic events"
...
With `start` anchored to midnight and a one-day period, `mode: 0.5` lands at noon; `left: 0.15` and `right: 0.9` concentrate activity between 03:36 and 21:36, tapering sharply outside that window. `ratio: 2000` sets about 2,000 logon-family events per day before the randomizer's ±25% variance is applied.
...
{%- set username = samples.usernames | random -%}
...
<Callout type="info">
To inspect events without writing to disk, replace the `file` output with [`stdout: {}`](/docs/plugins/output/stdout).
</Callout>

For a constant rate instead of a daily curve, a [cron](/docs/plugins/input/cron) input is simpler — it is what the Hub's own generators use. To emit Sysmon telemetry instead of Security events, keep the same input and output and swap in Sysmon-shaped templates: set `winlog.channel` to `Microsoft-Windows-Sysmon/Operational`, use Sysmon's own EventIDs, and add a `ProcessGuid` to correlate related events. The [Eventum Hub](/hub) ships both as ready-made, production-tuned generators: [windows-security](/hub/windows-security) covers 12 correlated event types across a 120-host Active Directory fleet, and [windows-sysmon](/hub/windows-sysmon) covers 15 Sysmon event types with process-lifecycle correlation across the same fleet.
...
<Accordions>
<Accordion title="Do I need a Windows host?">
No. The templates produce the same fields a Windows host would log, using ordinary sample data and randomization. Nothing here calls a Windows API or requires Sysmon, Active Directory, or a Windows machine of any kind — Eventum runs the same way on Linux, macOS, or in a container.
</Accordion>
