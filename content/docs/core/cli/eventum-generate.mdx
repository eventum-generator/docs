...
| `--batch.delay` | float | `1.0` | Maximum time span, in seconds, of the event timestamps one batch covers — see [when it applies](/docs/core/config/eventum-yml#generation). At least one of `--batch.size` or `--batch.delay` must be set. |
