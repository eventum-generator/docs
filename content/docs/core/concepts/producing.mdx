...
When a generator has **multiple templates**, the picking mode controls which templates render on each call:

| Mode | Behavior |
|------|----------|
| `all` | Renders every template, produces multiple events per timestamp |
| `any` | Randomly picks one template |
| `chance` | Picks one template based on weighted probabilities |
| `spin` | Cycles through templates in round-robin order |
| `chain` | Follows a user-defined sequence |
| `fsm` | Picks based on a finite state machine with conditional transitions |
