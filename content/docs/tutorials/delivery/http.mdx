...
Through the `auth` section, in whichever shape the destination expects. `type: bearer` sends a token that does not change; `type: basic` sends a user name and a password as HTTP basic credentials; `type: oauth2_client_credentials` names a token endpoint instead of a token, and Eventum takes a token from it, renews it before it expires and once more when a request comes back rejected — which is what Azure Monitor Logs Ingestion, Google SecOps and most hosted log APIs expect. Every credential can be written as a `${secrets.*}` reference resolved from the encrypted keyring at load time rather than kept as plain text. Beyond that, `client_cert`/`client_cert_key` cover mutual TLS, and `ca_cert` verifies the server's own certificate — see the [http reference](/docs/plugins/output/http#authentication) for the full shape of each method, and [Secrets](/docs/core/config/secrets) for how the keyring stores every credential referenced with `${secrets.*}`.
</Accordion>
