...
**Import** takes such an archive and unpacks it into a new project. The archive may hold `generator.yml` at its top level or nested in directories, so an archive downloaded from a repository imports as it is. The name is proposed from the file name and can be changed; an existing project is never overwritten.
