...
Managing live generators requires `allow_write: true`. Leave it `false` — the default — to let the agent only explore and preview. See the [configuration reference](/docs/core/config/eventum-yml#server-mcp) for every option.
</Step>
<Step>
### Start the server
