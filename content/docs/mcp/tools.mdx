...
| `rename_secret` | Write | Rename a secret, keeping its value under the new name. Connected repositories authenticating with it are repointed at the new name; generator configurations are not rewritten — check `list_secret_references` first. Refused when a repository already authenticates with the new name. |
