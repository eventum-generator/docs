...
Validating a CloudTrail parser, or a detection rule written against CloudTrail's own field names, needs event records shaped to the exact case those field names use — `eventName`, not `event_name` or `EventName`; `sourceIPAddress`, not `source_ip_address`. A real AWS account produces the genuine article, but triggering the specific API calls that generate a given event, then waiting for CloudTrail to actually deliver it, is slow for what it returns, and its events arrive on AWS's own schedule rather than a test's — nowhere near the varied, on-demand stream a parser or detection rule actually needs to be exercised against.

Eventum renders CloudTrail records directly from a template — the same flat camelCase fields, the same envelope CloudTrail itself writes to S3 — so a parser, a detection rule, or an ingestion pipeline built against CloudTrail's real field names can be exercised against a realistic stream of records without a live AWS account, a triggered API call, or a delivery delay anywhere in the loop.
...
Every field inside that one event is flat and camelCase — no nested schema namespaces the way [ECS](/docs/tutorials/formats/ecs) or [OCSF](/docs/tutorials/formats/ocsf) organize their own fields, aside from a handful of fields — `userIdentity` chief among them — that nest a small object of their own. A production trail holds many such objects inside one `Records` array per delivered file, not one file per event.
...
The generator below models a small multi-account AWS organization — deployments that assume a role far more often than anyone signs into the console by hand:
...
One string, one object, one named `Records` key holding an array of three complete records — the same envelope [the section above](#the-cloudtrail-record-format) described, now assembled from Eventum's own output instead of quoted from AWS's docs. Three shapes have appeared across this lesson, and a pipeline built against CloudTrail needs to know which one it is looking at: the per-record camelCase schema `json` produces above, the batched `Records` envelope `template-batch` reproduces here, and the ECS-normalized `aws.cloudtrail.*` document a collector like Filebeat's AWS module produces once it parses either one — covered in the [ECS lesson](/docs/tutorials/formats/ecs), not this page.
