...
description: How logs, metrics, and events differ, why a log and a metric are each an event in a different shape, and how to generate all three with Eventum.
---

A ticket asks for the metrics for the checkout service, and that request alone could mean a log line written each time an order fails, a number sampled every few seconds, or a structured record carrying a dozen named fields — three different things to generate, three different destinations, three different volumes. *Log*, *metric*, and *event* get used as if they name the same kind of thing, but a log store, a time-series database, and an event index each expect a specific shape, and a feed built for one doesn't load cleanly into another.
...
Three terms, three specific things:
...
These aren't three independent categories sitting side by side. A log is an event rendered as a line of text: the occurrence and the fields describing it are the same, only the shape changes, from named data to a sentence built for a person to read. A metric sample is an event too, one whose payload happens to be a measurement taken at a point in time rather than a description of what occurred. Events are the general shape; a log and a metric are two specializations of it, each suited to a different question — is this meant to be read on its own, or measured and compared across thousands of samples.
...
|--------|-------|------------------|--------------------|-------------------|
| Logs | A timestamped line of text | `2026-07-11T17:44:01Z ERROR order 39402 failed: payment declined` | A log store — a file, or a log-oriented search backend | Full-text or field search, filtered by a time range |
...
A single numeric measurement, sampled on a fixed interval, with no description attached beyond what's being measured and where. Eventum has no dedicated metrics format — a metric-shaped event is a regular JSON object built around a name and a value:
...
A record with several named fields, each describing one part of the same occurrence — closer to what an application or an API actually emits:
...
Yes, in the sense that matters here: a log is an event whose payload has been rendered as a line of text instead of kept as named fields. The occurrence being recorded is the same — what changes is whether a program reads a field by name or has to parse a sentence to find the same information. [Structured logging](/docs/tutorials/foundations/structured-logging) covers what's gained by keeping the fields instead of folding them into a sentence.
</Accordion>
...
- The [IoT sensor telemetry](/docs/tutorials/iot-telemetry) tutorial for a full generator producing drifting metric readings
