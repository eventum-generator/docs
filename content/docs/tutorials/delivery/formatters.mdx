...
description: Output formatters decide how the same event stream leaves each destination — one line per event, a single array, a syslog message, or a custom envelope — and which one to choose for a given target.
---
...
|-------------|-----------|
| Raw-text collector — a plain-text log file | [`plain`](/docs/plugins/formatters#plain) |
| A syslog listener — rsyslog, syslog-ng, a SIEM's syslog input | [`syslog`](/docs/plugins/formatters#syslog) |
...
A syslog listener accepts `plain` too, as long as the event template writes the whole `<PRI>`-prefixed line itself. `syslog` is what builds that header instead, from fields the event already carries — see the [syslog format lesson](/docs/tutorials/formats/syslog) for the header it produces and the [syslog transport lesson](/docs/tutorials/delivery/syslog-transport) for delivering it.
...
The formats split into two groups by what they operate on. `plain`, `json`, `template`, and `syslog` are per-event: each takes N events and returns up to N formatted strings, one per input event — `json` can skip an individual malformed event without touching the rest of the batch. `json-batch`, `template-batch`, and `eventum-http-input` are per-batch: each takes N events and returns exactly one formatted string for the whole batch, however many events that batch held.
