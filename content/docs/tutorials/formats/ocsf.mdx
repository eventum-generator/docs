...
In the raw output the JSON formatter escapes the ampersand in `category_name` as `\u0026` (a standard JSON escape that decodes back to `&`), which is why it reads `Identity \u0026 Access Management` above rather than a literal `&`.
