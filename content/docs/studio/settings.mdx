...
Renaming and removing a secret both state what refers to it first, and the two kinds of referrer are affected differently. A [connected repository](/docs/studio/repositories) authenticating with the secret is repointed at the new name by a rename, so it keeps working; a project reading it as `${secrets.name}` keeps the old name in its configuration and has to be edited. A removal repoints nothing — both kinds keep the name and stop working until a secret of that name exists again.

A rename is refused when a repository already authenticates with the new name, and names the repositories holding it. The keyring holds one value per name, so the repository left on that name would otherwise start authenticating with the value that just moved under it — the credential of one host reaching another.
