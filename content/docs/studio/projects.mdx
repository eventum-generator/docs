...
| Event | Template state | The live state of the plugin the debugger built |
...
**Start** builds an event plugin from the current configuration and holds it running; **Produce** renders events with it.
...
<Callout type="info">
The plugin compiles its templates when it starts. After editing a template, **Stop** and **Start** the debugger for the next run to use the new version.
</Callout>
