...
On a window narrower than 1200px the Explorer, the Editor and the Inspector stop sharing the row: one of them is shown at a time, chosen from a switcher above the workspace, and the Console starts collapsed to its header. Each panel keeps its contents while another one is on screen — open files, unsaved text and the work in the Console are all where they were on the way back. The dividers are unavailable in this layout.
