...
<Callout type="info">
An [MCP](/docs/mcp) agent can call `list_secrets` to discover which secret **names** exist — names only, never values — so it references them correctly in a config. Point the stdio server at the cryptfile with [`eventum mcp --keyring-cryptfile`](/docs/core/cli/eventum-mcp).
</Callout>
