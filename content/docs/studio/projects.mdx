...
The **Parameters** section holds the form for the selected plugin, validated against that [plugin's configuration](/docs/plugins) as you type. A rejected value is marked on its field with the reason beneath it. Fields that are not free text still accept `${params.name}` and `${secrets.name}` placeholders, which are resolved when an instance loads the configuration.
...
The panel can be collapsed to its header, or maximized over the rest of the workspace. Drag a divider inside a tool to give either neighboring pane more width; the tool starts from its default proportions again when the workspace is reloaded.
...
The output selector starts on the plugin open in the Inspector. Selecting any configured output loads its formatter into the preview. A plugin without an explicit `formatter` loads the default it uses at runtime. The preview also follows changes made to the selected output in the Inspector until you edit the preview itself. The status reads **Loaded** while the preview matches the selected output and **Custom** after you change it.

Loaded settings remain editable and do not change `generator.yml`. After an edit, **Reset** restores the selected output's current formatter and **Apply** sends the preview back to that output after confirmation. Applying updates the Studio draft while preserving the plugin's other settings. Use the main **Save** action to persist it to the project.

After the debugger produces events, **Replace with N debugger events** copies its latest successful result into the Events pane. The button is unavailable before the first run and states that it replaces the current list. The copied events remain editable, and events can still be added or removed by hand.
