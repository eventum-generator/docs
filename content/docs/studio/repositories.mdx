...
A generator the workspace already holds offers **Install again** instead. Installing it writes another project beside the one you have — the proposed name steps aside to a free one, and the project you already installed is left untouched. That is also how a generator the repository has changed is taken: the new version arrives as a separate project, to be compared against the one in use rather than overwriting it.
...
What the workspace already holds is stated by the action of the row — **Install again** rather than **Install** — so nothing repeats it beside the name. The one thing the action cannot say is marked instead: when the repository publishes the generator with content the installed project does not hold, the row carries **update available**.
