...
The user pool is a JSON file loaded as a [sample](/docs/plugins/event/template#samples). Each session randomly picks a user from this list. Sample rows are tuples following the JSON key order: `[0]` name, `[1]` domain.
...
{% do shared.set("username", user[0]) %}
{% do shared.set("domain", user[1]) %}
...
{% do shared.set("process_count", 0) %}
{
  ...
  "source": { "ip": "{{ src_ip }}" },
  "user": { "name": "{{ user[0] }}", "domain": "{{ user[1] }}" },
  "winlog": {
