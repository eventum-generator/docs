...
<div className="not-prose my-6 rounded-2xl border border-fd-border bg-linear-to-br from-fd-card via-fd-card to-fd-accent/20 p-8 text-center">
  <div className="flex justify-center mb-4 text-fd-muted-foreground"><Blocks size={40} strokeWidth={1.5} /></div>
  <div className="text-2xl font-semibold mb-2">Plugin reference</div>
...
| [otlp](/docs/plugins/output/otlp) | `plain` | Sends events as OpenTelemetry log records |
...
| [s3](/docs/plugins/output/s3) | `json` | Writes objects to S3 compatible storage |
