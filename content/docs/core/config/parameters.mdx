...
Each key in `params` maps to a `${params.<key>}` token in the generator config.
...
The JSON keys map to the same `${params.<key>}` tokens, and nested objects are read by the same path form. This is useful for one-off runs and CI pipelines where you don't want to create a `startup.yml`.
