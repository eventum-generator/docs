...
| `weighted_choice` | `(items, weights) → T` | Return a random item with weighted probability. Items and weights are two separate sequences of the same length. |
| `weighted_choices` | `(items, weights, n) → list[T]` | Return `n` items with weighted probability. |
